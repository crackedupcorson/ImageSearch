...
Naturally, there is still room for improvement in this regard as the search could be and 
needs to be more effective than it currently is.
...
class Query:
    ...
    def run(self,fileList):
        time_date = datetime
        estLocation = fileList['estLocation']
        relSearchTags = fileList['tags']
        image = fileList['Image']
        
         #Using Geocoder to get a geocode to use in flickr.getGeo(to only download images near gps given)
        ...
        try:
            for (k,v) in Image.open(image)._getexif().items(): #Kept in for testing purposes, and managing data
                    ...
                    pictureDate = get_field(exif,'DateTime')#Date Time
