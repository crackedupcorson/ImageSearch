...
def get_mintime_in_millis(amount_of_years):
    current_time = datetime.datetime.now()  # use datetime.datetime.utcnow() for UTC time
    x_years_ago = current_time - datetime.timedelta(days=amount_of_years * 365)
    milli_string = round(x_years_ago.timestamp() * 1000)
    print(milli_string)
    return milli_string
